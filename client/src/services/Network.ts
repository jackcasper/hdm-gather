...
export default class Network {
  ...
  constructor() {
    ...
    // Join the Lobby room when the Network instance is created
    this.joinLobbyRoom().then(() => {
      store.dispatch(setLobbyJoined(true))
    })

    // Event listeners for player name and texture changes, and player disconnection
  ...
  // Method to connect to a computer
  connectToComputer(id: string) {
    this.room?.send(Message.CONNECT_TO_COMPUTER, { computerId: id })
  }

  // Method to disconnect from a computer
  disconnectFromComputer(id: string) {
    this.room?.send(Message.DISCONNECT_FROM_COMPUTER, { computerId: id })
  }

  // Method to connect to a whiteboard
  connectToWhiteboard(id: string) {
    this.room?.send(Message.CONNECT_TO_WHITEBOARD, { whiteboardId: id })
  }

  // Method to disconnect from a whiteboard
  disconnectFromWhiteboard(id: string) {
    this.room?.send(Message.DISCONNECT_FROM_WHITEBOARD, { whiteboardId: id })
  }

  // Method to stop sharing the screen in a computer
